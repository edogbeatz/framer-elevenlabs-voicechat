...
class HeatmapShaderMount {
    ...
    private handleResize(width: number, height: number) {
        ...
        // Maintain 4:3 aspect ratio - calculate dimensions that fit in container
        const canvasAspectRatio = 4 / 3
        let canvasWidth = width
        let canvasHeight = width / canvasAspectRatio
...
// ============================================
// Audio Heatmap Framer Component
// ============================================

export function AudioHeatmap(props: AudioHeatmapProps) {
    const {
        image,
        colors = [
            "#11206A",
    ...
    React.useEffect(() => {
        ...
        try {
            ...
            // Only animate in Preview mode, keep static on Canvas
            if (!isCanvas) {
                shader.start()
                console.log("[AudioHeatmap] Shader animation started (Preview)")
            } else {
                // Render a single static frame for Canvas
                shader.render(performance.now())
                console.log(
                    "[AudioHeatmap] Shader static frame rendered (Canvas)"
                )
            }
...
addPropertyControls(AudioHeatmap, {
    image: {
    ...
    colorBack: {
        ...
        title: "Background",
        defaultValue: "#050505",
        description: "Background color behind the heatmap",
        group: "Colors",
    },
    speed: {
    ...
    fit: {
        ...
        description: "How the image fits in the canvas",
        group: "Layout",
    },
    audioReactivity: {
        type: ControlType.Number,
        ...
        max: 3,
        step: 0.1,
        description: "Overall audio reactivity intensity",
...
export default React.memo(AudioHeatmap)
